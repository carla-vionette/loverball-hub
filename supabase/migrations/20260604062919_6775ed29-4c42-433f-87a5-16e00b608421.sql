...
CREATE OR REPLACE FUNCTION public.admin_get_event_attendees(p_event_id uuid)
 RETURNS TABLE(id uuid, status text, user_id uuid, plus_ones integer, guest_name text, guest_phone text, created_at timestamp with time zone, profile_name text, profile_city text, profile_photo_url text, profile_instagram_url text, profile_email text)
 LANGUAGE plpgsql
