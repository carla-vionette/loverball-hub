  ...
  useEffect(() => {
    (async () => {
      ...
      const ranked = (peopleData || [])
        .map((p) => {
          const teamsP: string[] = Array.isArray((p as any).favorite_la_teams) ? (p as any).favorite_la_teams : [];
          const overlap = teamsP.filter((t) => myTeams.includes(t)).length;
          return { ...p, _overlap: overlap };
        })
        .sort((a: any, b: any) => b._overlap - a._overlap)
        .slice(0, 6);
      setPeople(ranked as Person[]);

      const evQuery = supabase
        .from("events")
        .select("id, title, event_date, city, image_url, banner_image, event_type, sport_tags")
    ...
    <div className="min-h-[100dvh] flex flex-col" style={{ background: C.bg, color: C.text, fontFamily: fonts.sans }}>
      ...
      <main className="flex-1 max-w-md w-full mx-auto px-5 pt-10 pb-32">
        ...
        {events.length > 0 && (
          <section className="mb-10">
            ...
            <div className="space-y-3">
              {events.map((ev) => {
                const eventImage = resolveEventImage(ev);
                return (
                  <button
                    key={ev.id}
        ...
        <div className="grid grid-cols-2 gap-3">
          <Button className="h-12" onClick={() => navigate("/friends")}>Find your people</Button>
          <Button variant="outline" className="h-12" onClick={() => navigate("/messages")}>
            <MessageCircle className="w-4 h-4 mr-2" /> Open chats
          </Button>
        </div>
      </main>
    </div>
