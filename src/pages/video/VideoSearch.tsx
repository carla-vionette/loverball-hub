        ...
        <div className="flex items-center gap-2 px-4 md:px-8 pb-3 overflow-x-auto scrollbar-hide">
          ...
          <button
            ...
            className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold transition-all ${
              showFilters ? "bg-foreground text-background" : "bg-card text-foreground border border-border/40 hover:bg-secondary"
            }`}
          >
              ...
              <div className="flex flex-wrap gap-2">
                {TRENDING_SEARCHES.map((term, i) => (
                  <button
                    key={term}
                    onClick={() => handleSearch(term)}
                    className="group flex items-center gap-2 px-4 py-2 bg-card text-foreground border border-border/40 text-xs rounded-full hover:bg-secondary hover:text-primary transition-all"
                  >
                    <Flame className="w-3 h-3 text-primary/70 group-hover:text-primary transition-colors" />
                    <span className="text-foreground/50 font-bold mr-0.5">{i + 1}</span>
                    {term}
                  </button>
                ))}
              </div>
