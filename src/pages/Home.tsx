...
const Home = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* HERO — Full bleed coral gradient overlay */}
      <section className="relative min-h-[95vh] flex items-end overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12 pb-16 md:pb-24 w-full">
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="max-w-3xl">
            ...
            <h1 className="text-6xl sm:text-7xl lg:text-8xl font-display leading-none mb-6 text-white">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Button size="lg" onClick={() => navigate("/")} className="rounded-full bg-white text-primary hover:bg-white/90 px-10 py-6 text-base font-sans tracking-widest uppercase shadow-xl">
      ...
      {/* FEATURED EVENTS — Lavender section */}
      {featuredEvents.length > 0 && (
        <section className="py-20 bg-lavender">
          <div className="max-w-7xl mx-auto px-6 lg:px-12">
            <div className="flex items-end justify-between mb-10">
              <div>
                <p className="text-xs font-sans tracking-[0.3em] uppercase text-lavender-foreground/50 mb-3">What's Next</p>
                <h2 className="font-display text-4xl md:text-5xl text-lavender-foreground italic">Featured Events</h2>
              </div>
              <div className="flex gap-2">
                <Button variant="outline" size="icon" className="rounded-full border-lavender-foreground/20 text-lavender-foreground hover:bg-lavender-foreground/10" onClick={prevSlide}><ChevronLeft className="w-4 h-4" /></Button>
                <Button variant="outline" size="icon" className="rounded-full border-lavender-foreground/20 text-lavender-foreground hover:bg-lavender-foreground/10" onClick={nextSlide}><ChevronRight className="w-4 h-4" /></Button>
              </div>
            </div>
              ...
              >
                {featuredEvents.map(event => (
                  <div key={event.id} className="min-w-[calc(33.333%-16px)] max-w-[calc(33.333%-16px)] flex-shrink-0 max-md:min-w-[calc(100%-16px)] max-md:max-w-[calc(100%-16px)]">
                    <Card className="overflow-hidden cursor-pointer group border-0 shadow-lg" onClick={() => navigate(`/event/${event.id}`)}>
                      <div className="relative h-52 bg-muted overflow-hidden">
                        ...
                        {event.event_type && (
                          <Badge className="absolute top-3 left-3 bg-primary text-primary-foreground rounded-full font-sans font-bold tracking-wider text-xs">{event.event_type}</Badge>
                        )}
                      </div>
      ...
      <section className="py-24 bg-background">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="text-center mb-16">
            ...
            <h2 className="font-display text-5xl md:text-6xl text-foreground">Getting <em className="text-primary">Started</em></h2>
          </div>
          <div className="grid md:grid-cols-3 gap-8">
            {howItWorks.map((step, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: i * 0.15 }} viewport={{ once: true }}>
                <div className="border border-border rounded-2xl p-8 h-full hover:border-primary transition-colors duration-300 hover:shadow-lg">
      ...
      {/* LATEST VIDEOS — Coral section */}
      <section className="py-20 bg-primary">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="flex items-end justify-between mb-10">
            <div>
              <p className="text-xs font-sans tracking-[0.3em] uppercase text-primary-foreground/50 mb-3">Watch Now</p>
              <h2 className="font-display text-4xl md:text-5xl text-primary-foreground italic">Latest Videos</h2>
            </div>
            <Button variant="outline" className="rounded-full font-sans tracking-widest uppercase text-xs border-white/30 text-white hover:bg-white/10 bg-transparent" onClick={() => navigate("/watch")}>View All <ArrowRight className="ml-1 w-4 h-4" /></Button>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {latestVideos.map((video, i) => (
              <Card key={video.id} className={`overflow-hidden cursor-pointer group border-0 bg-white/10 backdrop-blur-sm ${i === 0 ? 'sm:col-span-2 sm:row-span-2' : ''}`} onClick={() => navigate("/watch")}>
                <div className={`relative ${i === 0 ? 'aspect-[4/3]' : 'aspect-video'} bg-black/20 overflow-hidden`}>
                  ...
                  {video.duration && (
                    <span className="absolute bottom-2 right-2 bg-black/70 text-white text-[10px] px-1.5 py-0.5 rounded font-sans font-bold">{Math.floor(video.duration / 60)}:{String(video.duration % 60).padStart(2, '0')}</span>
                  )}
                </div>
                <CardContent className="pt-3 pb-4">
                  <h4 className="text-sm font-display line-clamp-2 text-primary-foreground group-hover:text-white transition-colors normal-case">{video.title}</h4>
                  <p className="text-xs text-primary-foreground/60 mt-1 font-sans">{video.channelName}</p>
                </CardContent>
              </Card>
            ))}
          </div>
      ...
      <section className="py-24 bg-lavender">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="text-center mb-14">
            <p className="script-accent text-primary mb-2">community love</p>
            <h2 className="font-display text-5xl md:text-6xl text-lavender-foreground">What Members <em className="text-primary">Say</em></h2>
          </div>
          <div className="grid md:grid-cols-3 gap-8">
            {testimonials.map((t, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: i * 0.1 }} viewport={{ once: true }}>
                <div className={`p-8 sm:p-10 h-full flex flex-col rounded-2xl ${i === 0 ? 'bg-primary text-primary-foreground' : i === 1 ? 'bg-white text-foreground shadow-lg' : 'bg-accent text-accent-foreground'}`}>
                  <span className="font-display text-5xl leading-none opacity-20 mb-4">"</span>
                  <p className={`leading-relaxed flex-1 mb-6 text-lg font-serif italic ${i === 0 ? 'text-primary-foreground/90' : i === 1 ? 'text-foreground/80' : 'text-accent-foreground/90'}`}>"{t.quote}"</p>
                  <div>
                    <p className={`font-sans font-bold text-sm`}>{t.name}</p>
                    <p className={`text-xs font-sans opacity-60`}>{t.role}</p>
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      <section className="py-20 bg-background">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="text-center mb-14">
            ...
            <h2 className="font-display text-5xl md:text-6xl text-foreground">Teams We <em className="text-primary">Love</em></h2>
          </div>
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-5">
            {LA_TEAMS.map(team => (
              <motion.div key={team.name} whileHover={{ scale: 1.06, y: -4 }} className="flex flex-col items-center gap-3 p-5 bg-card border border-border rounded-2xl hover:border-primary hover:shadow-lg transition-all cursor-pointer">
        ...
        <div className="max-w-4xl mx-auto px-6 lg:px-12 text-center">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button size="lg" onClick={() => navigate("/")} className="rounded-full text-lg px-10 py-7 bg-white text-primary hover:bg-white/90 font-sans tracking-widest uppercase shadow-xl">
                Join Loverball <ArrowRight className="ml-2 h-5 w-5" />
              </Button>
              <Button size="lg" variant="outline" onClick={() => navigate("/events")} className="rounded-full text-lg px-10 py-7 border-2 border-white/30 text-primary-foreground hover:bg-white/10 bg-transparent font-sans tracking-widest uppercase">
                Explore Events
              </Button>
            </div>
          </motion.div>
