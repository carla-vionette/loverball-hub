...
import { CheckCircle, Users, Play, ArrowLeft, Loader2, Bell, BellOff } from "lucide-react";
...
const ChannelProfile = () => {
  ...
  const fetchChannel = async () => {
    ...
    try {
      ...
      if (data) {
        setChannel(data);
        setSubCount(data.follower_count);
  ...
  const initials = channel.channel_name.split(" ").map((w) => w[0]).join("").slice(0, 2).toUpperCase();
  const typeLabel = channel.channel_type === "team" ? "Team" : channel.channel_type === "creator" ? "Creator" : "Official";
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="md:ml-64 pt-16 md:pt-0 pb-24 md:pb-0">
        <div className="max-w-3xl mx-auto px-5 md:px-10 py-6">
          <Button variant="ghost" className="gap-2 mb-4 -ml-2 text-muted-foreground hover:text-foreground" onClick={() => navigate(-1)}>
          ...
          <div className="flex flex-col items-center text-center gap-4 mb-8">
            <Avatar className="w-24 h-24">
              ...
              <AvatarFallback className={`${CHANNEL_TYPE_COLORS[channel.channel_type] || "bg-primary"} text-white font-bold text-2xl`}>
                {initials}
              </AvatarFallback>
            </Avatar>
            <div>
              <div className="flex items-center justify-center gap-1.5 mb-1">
                <h1 className="font-condensed text-2xl font-bold uppercase tracking-wide">{channel.channel_name}</h1>
              ...
              <div className="flex items-center justify-center gap-3 mb-3 flex-wrap">
                <Badge className="bg-primary/10 text-primary text-xs font-bold border-0 rounded-full">{typeLabel}</Badge>
                {channel.league && <Badge variant="outline" className="text-xs rounded-full">{channel.league}</Badge>}
                {channel.sport_focus && <Badge variant="outline" className="text-xs rounded-full">{channel.sport_focus}</Badge>}
                <span className="text-sm text-muted-foreground flex items-center gap-1">
                  <Users className="w-3.5 h-3.5" /> {subCount.toLocaleString()} followers
                </span>
              </div>
              {channel.description && <p className="text-sm text-muted-foreground max-w-md">{channel.description}</p>}
              {channel.location && <p className="text-xs text-muted-foreground mt-1">{channel.location}</p>}
            </div>
          ...
          <h2 className="font-condensed text-lg font-bold uppercase tracking-wide mb-4">Videos ({videos.length})</h2>
          {videos.length === 0 ? (
            ...
            <div className="grid grid-cols-3 gap-2">
              {videos.map((video) => (
                <a key={video.id} href={`/watch/video/${video.id}`} className="cursor-pointer group block">
                  <div className="relative w-full aspect-[9/16] rounded-xl overflow-hidden bg-muted">
                    {(video.thumbnail || video.thumbnail_url) ? (
                      <img src={video.thumbnail || video.thumbnail_url || ""} alt={video.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" loading="lazy" />
                    ) : (
