                            ...
                            return (
                              <>
                                ...
                                {(() => {
                                  ...
                                  const bars = watchBarsForCity(ev.city);
                                  return (
                                    <div className="pt-2 space-y-2" onClick={(e) => e.stopPropagation()}>
                                      <div className="px-3 py-2 rounded-xl space-y-1.5"
                                        ...
                                        <div className="pt-1.5">
                                          <p style={{ fontFamily: "'Inter', sans-serif", fontWeight: 700, fontSize: 10, letterSpacing: "0.1em", textTransform: "uppercase", color: "#FAF5E9", margin: 0 }}>
                                            🍻 Where to Watch{!isLA && ev.city ? ` in ${ev.city}` : isLA ? " in LA" : ""}
                                          </p>
                                          {bars.length > 0 ? (
                                            <ul style={{ listStyle: "none", padding: 0, margin: "4px 0 0" }}>
                                              {bars.slice(0, 4).map((b) => (
                                                <li key={b.name} style={{ fontFamily: "'Space Mono', monospace", fontSize: 10.5, color: "rgba(248,248,248,0.75)", lineHeight: 1.6 }}>
                                ...
                                {(() => {
                                  const LOCAL_STADIUM_RADIUS_MI = 50;
                                  const isLocalGame = dist != null && dist <= LOCAL_STADIUM_RADIUS_MI;
                                  return (
                                    <>
                                      {!isLocalGame && (
                                        <div className="pt-2" onClick={(e) => e.stopPropagation()}>
                                          <p style={{ fontFamily: "'Space Mono', ui-monospace, monospace", fontSize: 9.5, color: "rgba(45,212,191,0.85)", letterSpacing: "0.12em", textTransform: "uppercase", margin: 0 }}>
                                            🍺 Watch party only — stadium not nearby{dist != null ? ` (${Math.round(dist)} mi)` : ""}
                                          </p>
                                        </div>
                                      )}
                                      <div className={`grid ${isLocalGame ? "grid-cols-2" : "grid-cols-1"} gap-2 pt-2`} style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }} onClick={(e) => e.stopPropagation()}>
                                        {isLocalGame && wrap(
                                          <Button
                                            size="sm"
                                            disabled={!user}
