  ...
  useEffect(() => {
    ...
    fetchVideoById(id)
      .then((v) => {
        setVideo(v);
        if (v) {
          fetchRelatedVideos(v.id, v.category).then(setRelated);
        }
      })
      .catch(console.error)
    ...
    return (
      <AppLayout>
        <div className="container mx-auto px-4 py-16 text-center">
          ...
          <Button onClick={() => navigate('/watch')}>Back to Library</Button>
        </div>
      </AppLayout>
    );
  }
  ...
  return (
    <AppLayout>
      <div className="container mx-auto px-4 py-8 max-w-5xl">
        ...
        {video.tier !== 'free' ? (
          <GatedContent requiredTier={video.tier}>
            {videoContent}
          </GatedContent>
        ) : (
          videoContent
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            {video.tier !== 'free' && (
              <Badge className={video.tier === 'premium' ? 'bg-purple-500/10 text-purple-600' : 'bg-primary/10 text-primary'}>
